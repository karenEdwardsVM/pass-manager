...
;   working to display Hello World in the View area.
...
(define (display-cb l from-f to-f)
  (lambda (elt e)
...
(define pass-entry (create-text-field "Pass it here" input-frame (write-data-on-enter "testing" input-frame display-frame)))
(define enter-butt (create-button "Enter" input-frame (write-data-on-enter "testing" input-frame display-frame)))
; make text-field to display current passwords with enabled set to false
(define display-passwords (create-text-field "Display Here" display-frame ;(swap-frames opening-frame display-frame) #f))
                            (display-cb (read-passwords FILE) opening-frame display-frame) #f "Hello World")) ;(style-printing (first test-list)))) ; "Hello World"))
